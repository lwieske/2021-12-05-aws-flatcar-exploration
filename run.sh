...
export PREFIX=AWS_FLATCAR_EXPLORATION
...
ssh -o StrictHostKeyChecking=no -i ${KEY_NAME}.pem core@${INSTANCE_PUBLIC_IP} -tt <<EOF
    set -x
    /opt/bin/containerd --version
    /opt/bin/crictl --version
    /opt/bin/runc --version
    ls -la /opt/cni/bin
    sudo crictl ps
    sleep 20
    clear
    ...
    /opt/bin/kubectl version --client=false
    sleep 20
    clear
    systemctl status containerd --lines=0 | head -5 
    systemctl status kubelet    --lines=0 | head -5
    sleep 20
    exit
EOF
